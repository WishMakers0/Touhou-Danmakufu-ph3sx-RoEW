...
//ObjMtlMeshData
ObjMtlMeshData::ObjMtlMeshData() {}
ObjMtlMeshData::~ObjMtlMeshData() {
	for (auto& obj : renderList_) ptr_delete(obj);
	for (auto& obj : materialList_) ptr_delete(obj);
}
bool ObjMtlMeshData::CreateFromFileReader(shared_ptr<gstd::FileReader> reader) {
	bool res = false;
	...
	try {
		...
		Logger::WriteTop(StringUtility::Format(L"ObjMtlMeshData parsing error. [line %d-> %s]", 
			scanner.GetCurrentLine(), e.what()));
		res = false;
	}
	return res;
}
void ObjMtlMeshData::_ReadMaterial(gstd::Scanner& scanner) {
...
void ObjMtlMeshData::_ReadObject(gstd::Scanner& scanner) {
	Object obj;
	...
	std::map<int, std::list<ObjMtlMeshData::Object::Face*>> mapFace;
		...
		while (true) {
			...
			if (tok.GetElement() == L"visible") {
				...
				for (size_t iFace = 0; iFace < countFace; ++iFace) {
					...
					ObjMtlMeshData::Object::Face* face = &obj.faces_[iFace];
	...
	for (auto itrMap = mapFace.begin(); itrMap != mapFace.end(); itrMap++, iMapFace++) {
		...
		std::list<ObjMtlMeshData::Object::Face*>& listFace = itrMap->second;

		ObjMtlMeshData::RenderObject* render = new ObjMtlMeshData::RenderObject();
		...
		for (auto itrFace = listFace.begin(); itrFace != listFace.end(); ++itrFace) {
			ObjMtlMeshData::Object::Face* face = *itrFace;
...
ObjMtlMeshData::Material* ObjMtlMeshData::RenderObject::nullMaterial_ = new Material();
void ObjMtlMeshData::RenderObject::Render(D3DXMATRIX* matTransform) {
	IDirect3DDevice9* device = DirectGraphics::GetBase()->GetDevice();
...
//ObjMtlMesh
bool ObjMtlMesh::CreateFromFileReader(shared_ptr<gstd::FileReader> reader) {
	bool res = false;
	{
		...
		if (data_ == nullptr) {
			if (!reader->Open()) throw gstd::wexception("ObjMtlMesh: Cannot open file for reading.");
			data_ = std::make_shared<ObjMtlMeshData>();
			data_->SetName(name);
			ObjMtlMeshData* data = (ObjMtlMeshData*)data_.get();
			res = data->CreateFromFileReader(reader);
			if (res) {
				Logger::WriteTop(StringUtility::Format(L"ObjMtlMesh: Mesh loaded. [%s]", name.c_str()));
...
bool ObjMtlMesh::CreateFromFileInLoadThread(const std::wstring& path) {
	return DxMesh::CreateFromFileInLoadThread(path, MESH_METASEQUOIA);
}
std::wstring ObjMtlMesh::GetPath() {
	if (data_ == nullptr) return L"";
	return ((ObjMtlMeshData*)data_.get())->path_;
}

void ObjMtlMesh::Render() {
	ObjMtlMesh::Render(D3DXVECTOR2(1, 0), D3DXVECTOR2(1, 0), D3DXVECTOR2(1, 0));
}
void ObjMtlMesh::Render(const D3DXVECTOR2& angX, const D3DXVECTOR2& angY, const D3DXVECTOR2& angZ) {
